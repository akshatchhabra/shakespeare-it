...
def load_test_set():
    neg_yelp = load_dataset('../data/sentiment.test.0')
    pos_yelp = load_dataset('../data/sentiment.test.1')
    yelp_x_test, yelp_y_test = compile_binary_dataset(neg_yelp, pos_yelp)
    return yelp_x_test, yelp_y_test

def load_train_set():
    neg_yelp = load_dataset('../data/sentiment.train.0')
    pos_yelp = load_dataset('../data/sentiment.train.1')
...
def calculate_corpus_level_scores(model, aspect):    
    ...
    automated_sentence_level_scores_base_path = f'../evaluations/automated/{aspect}/sentence_level'
    ...
    for val in param_values:
        ...
        sentence_level_scores_path = f'{automated_sentence_level_scores_base_path}/{model}_{param_name}_{string_val}.npz'
